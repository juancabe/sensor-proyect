...
#[cfg(test)]
mod test {
    use crate::state::poisonable_identifier::PoisonableIdentifier;

    #[test]
    fn test_poison() {
        let username = "paquito";
        let id = PoisonableIdentifier::Username(username.into());
        id.poison().expect("Should be able to poison");
        assert!(id.is_poisoned().expect("Should be able to check"));
    }
}
